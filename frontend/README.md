...
| 6 | `sam build` for `infra/api-gateway.yaml` fails: `strands-agents` (Day 2) pulls in `mcp`, which has a Windows-only `pywin32` dependency the local pip resolver can't satisfy for the Lambda target; `--use-container` gets past that but then fails `CopySource` on `.claude/skills/...`, a symlink to an absolute host path that doesn't exist inside the container mount, despite `.samignore` listing `.claude/`. Latent since Day 2 — nobody had rebuilt this template since `strands-agents` was added. | Lane A / infra | Any future change to `infra/api-gateway.yaml` needs a real `sam deploy` to reach the live stack (blocker 4 was worked around with a direct API call instead). |
