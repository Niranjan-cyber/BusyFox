...
| 3 | The deployed stack has no DynamoDB table at all — the only successful `sam deploy` predates Task 20 adding one to the template, and `sam build` has been broken since (blocker 6) so nothing since has redeployed. Real Lambda code (this session's fixes included) and real data are both still one `sam deploy` away from the live stack. | Lane A | A "live" response today is a fixture served over HTTP. Blocker 1 is what makes that visible rather than mislabelled. |
